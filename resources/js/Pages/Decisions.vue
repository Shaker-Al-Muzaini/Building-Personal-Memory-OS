...
const finalizeDecision = (id, choice) => {
    ...
    router.patch(route('decisions.finalize', id), { final_decision: choice }, { 
        ...
        onSuccess: () => {
            ...
            Swal.fire({
                title: trans('Great!'),
                text: trans('Your decision has been sealed.'),
                icon: 'success',
                timer: 2500,
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-2xl text-accent leading-tight flex items-center gap-2">
                <span>⚖️</span> {{ $t('Decisions Memory') }}
            </h2>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-xl relative">
                    ...
                    <div class="relative z-10">
                        <h3 class="text-3xl font-bold text-white mb-2">{{ $t('I am confused... help me!') }}</h3>
                        <p class="text-gray-400 mb-6">{{ $t('Describe problem...') }}</p>
                        ...
                        <form @submit.prevent="saveProblem" class="flex flex-col gap-4">
                            ...
                            >
                                ...
                                <span>{{ decisionForm.processing ? $t('Thinking...') : $t('Ask Advisor') }}</span>
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <!-- Decisions List -->
                <TransitionGroup name="list" tag="div" class="space-y-6 pb-20">
                    <div v-for="decision in decisions" :key="decision.id" class="bg-black bg-opacity-30 border border-gray-800 rounded-2xl p-6 relative group transition duration-300">
                        ...
                        <div class="p-4 bg-gray-900 border border-gray-800 rounded-xl mt-4" v-if="decision.ai_advice">
                            <div class="flex items-center gap-2 mb-2 text-accent text-lg">
                                <span>🤖</span>
                                <span class="font-bold">{{ $t('AI Advisor Opinion:') }}</span>
                            </div>
                        ...
                        <div class="mt-6 pt-4 border-t border-gray-800" v-if="!decision.final_decision">
                            <label class="block text-sm text-gray-400 mb-2 font-bold">{{ $t('What is your final decision?') }}</label>
                            <div class="flex gap-2">
                                <input type="text" v-model="choices[decision.id]" placeholder="..." class="flex-1 bg-black bg-opacity-30 border border-gray-700 rounded-lg px-3 py-2 text-white focus:ring-accent text-sm" @keyup.enter="finalizeDecision(decision.id, choices[decision.id])" />
                                <button @click="finalizeDecision(decision.id, choices[decision.id])" class="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition">{{ $t('Finalize Decision') }}</button>
                            </div>
                        </div>
                        
                        <div class="mt-6 p-4 bg-green-900 border border-green-800 rounded-xl text-green-100" v-else>
                            <span class="font-bold block mb-1">{{ $t('Final Decision Made:') }}</span>
                            {{ decision.final_decision }}
                        </div>

                    </div>

                    <div v-if="decisions.length === 0" class="text-center py-12 text-gray-500" key="empty">
